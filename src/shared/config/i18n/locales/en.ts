// src/shared/config/i18n/locales/en.ts
const en = {
    challengeActions: {
        join: {
            success: 'You have joined the challenge!',
            error: 'Failed to join challenge.',
        },
        submit: {
            success: 'Your completion has been submitted for verification!',
            error: 'Failed to submit completion. Please try again.',
        },
        delete: {
            title: 'Delete Quest',
            message: 'Are you sure you want to delete this quest? This action cannot be undone.\n\nNote: Statistics from completed sessions will be preserved.',
            confirm: 'Delete',
            success: 'Quest deleted successfully',
            error: 'Failed to delete quest. Please try again.',
            errorForbidden: "You don't have permission to delete this quest",
            errorNotFound: 'Quest not found',
        },
        invitation: {
            success: 'Invitation sent successfully!',
            error: 'Failed to send invitation.',
            tabs: {
                received: 'Received',
                sent: 'Sent',
            },
            noInvitations: 'No invitations found',
            card: {
                from: 'From: {{username}}',
                to: 'To: {{username}}',
                negotiationInProgress: 'Negotiation in progress',
                expired: 'Expired',
                status: {
                    PENDING: 'PENDING',
                    ACCEPTED: 'ACCEPTED',
                    DECLINED: 'DECLINED',
                    NEGOTIATING: 'NEGOTIATING',
                    EXPIRED: 'EXPIRED',
                    CANCELLED: 'CANCELLED',
                },
            },
            details: {
                questLabel: 'Quest',
                fromLabel: 'From',
                toLabel: 'To',
                stakesTitle: 'Stakes',
                typeLabel: 'Type',
                amountLabel: 'Amount',
                socialPenaltyLabel: 'Social Penalty',
                activeNegotiationTitle: 'Active Negotiation',
                counterOfferButton: 'Counter-Offer',
                declineButton: 'Decline',
                acceptButton: 'Accept',
                cancelInvitationButton: 'Cancel Invitation',
            },
            negotiation: {
                counterOfferTitle: 'Counter Offer',
                proposedBy: 'Proposed by {{username}}',
                typeLabel: 'Type:',
                amountLabel: 'Amount:',
                penaltyLabel: 'Penalty:',
                waitingForResponse: 'Waiting for response...',
                rejectButton: 'Reject',
                acceptButton: 'Accept',
            },
            modal: {
                title: 'Invite to "{{title}}"',
                userIdLabel: 'User ID to Invite',
                userIdPlaceholder: 'Enter User ID',
                checkingPrivacy: 'Checking privacy...',
                cannotInvite: 'User cannot be invited.',
                minutesLabel: 'Minutes',
                minutesPlaceholder: 'e.g. 30',
                penaltyLabel: 'Penalty Description',
                penaltyPlaceholder: 'What must the loser do?',
                amountLabel: 'Amount',
                amountPlaceholder: '0',
                expiresAtLabel: 'Expires At',
                messageLabel: 'Message (Optional)',
                messagePlaceholder: 'Add a note...',
                cancelButton: 'Cancel',
                sendButton: 'Send Invite',
                errorTitle: 'Error',
                errorUserId: 'Please enter a user ID',
                errorCannotInvite: 'This user does not accept invitations from you.',
                errorStake: 'Please enter a stake amount',
            },
        },
        creator: {
            errorInfoNotAvailable: 'Creator information is not available',
        },
    },
    common: {
        all: 'All',
        loading: 'Loading...', 
        error: 'Error',
        success: 'Success',
        cancel: 'Cancel',
        save: 'Save',
        delete: 'Delete',
        edit: 'Edit',
        back: 'Back',
        next: 'Next',
        done: 'Done',
        retry: 'Retry',
        yes: 'Yes',
        no: 'No',
        ok: 'OK',
        search: 'Search',
        noResults: 'No results found',
        close: 'Close',
        goBack: 'Go Back',
        seconds: '{{count}}s',
        hide: 'Hide',
        showHint: 'Show Hint',
        start: 'Start',
        continue: 'Continue',
        failed: 'Failed',
        localizedInput: {
            fillAtLeastOne: 'Please fill in at least one language',
            fillBothLanguages: 'Please fill in both languages',
        },
        difficulty: {
            easy: 'Easy',
            medium: 'Medium',
            hard: 'Hard',
        },
        visibility: {
            public: 'Public',
            private: 'Private',
            friendsOnly: 'Friends Only',
            groupOnly: 'Group Only',
        },
    },
    locationQuest: {
        discovery: {
            title: 'Discover Quests',
            nearYou: 'Quests Near You',
            noQuests: 'No quests found nearby',
            noQuestsHint: 'Try expanding your search area',
            permissionDenied: 'Location permission is required to find nearby quests',
            openSettings: 'Open Settings',
            distance: '{{distance}} km away',
            waypoints: '{{count}} waypoints',
            estimatedTime: '~{{minutes}} min',
            participants: '{{count}} participants',
            pullToRefresh: 'Pull to refresh',
            errorLoading: 'Failed to load quests',
        },
        detail: {
            title: 'Quest Details',
            joinQuest: 'Join Quest',
            joining: 'Joining...',
            continueQuest: 'Continue Quest',
            estimatedTime: 'Estimated Time',
            totalDistance: 'Total Distance',
            waypoints: 'Waypoints',
            participants: 'Participants',
            prizePool: 'Prize Pool',
            createdBy: 'Created by {{username}}',
            waypointList: 'Route Overview',
            alreadyJoined: 'Already joined',
            errorLoading: 'Failed to load quest details',
            joinFailed: 'Failed to join quest. Please try again.',
            questFull: 'This quest is full',
        },
        active: {
            currentWaypoint: 'Current Waypoint',
            nextWaypoint: 'Next',
            distanceRemaining: '{{distance}}m remaining',
            arrived: "You've arrived!",
            confirmArrival: 'Confirm Arrival',
            confirming: 'Confirming...',
            timeRemaining: 'Time: {{time}}',
            progressLabel: '{{completed}}/{{total}} waypoints',
            abandon: 'Abandon Quest',
            abandonConfirmTitle: 'Abandon Quest?',
            abandonConfirmMessage: 'Your progress will be lost. Are you sure?',
            abandonConfirm: 'Yes, Abandon',
            questComplete: 'Quest Complete!',
            congratulations: 'Congratulations! You completed all waypoints.',
            backToDiscovery: 'Back to Discovery',
            gpsUnavailable: 'GPS signal unavailable',
            wsDisconnected: 'Real-time updates paused',
            connecting: 'Connecting...',
        },
        map: {
            waypointVisited: 'Visited',
            waypointCurrent: 'Current',
            waypointUpcoming: 'Upcoming',
            yourLocation: 'You',
        },
        tasks: {
            gpsCheckin: {
                title: 'Location Verified',
                subtitle: 'You have arrived at the checkpoint!',
                completed: 'Check-in complete',
            },
            coinJingle: {
                title: 'Coin Jingle',
                instructions: 'Shake a coin near the microphone for 3 seconds',
                startRecording: 'Start Recording',
                recording: 'Recording...',
                countdown: '{{seconds}}s remaining',
                playback: 'Play Recording',
                reRecord: 'Re-record',
                submit: 'Submit Recording',
                permissionDenied: 'Microphone access is required for this task',
                openSettings: 'Open Settings',
                recordingFailed: 'Recording failed. Please try again.',
            },
            videoSelfie: {
                title: 'Video Selfie',
                instructions: 'Record a 5-second video selfie at this location',
                startRecording: 'Start Recording',
                recording: 'Recording...',
                countdown: '{{seconds}}',
                preview: 'Preview',
                reRecord: 'Re-record',
                submit: 'Submit Video',
                permissionDenied: 'Camera and microphone access are required',
                recordingFailed: 'Video recording failed. Please try again.',
            },
            photoRecreation: {
                title: 'Photo Recreation',
                instructions: 'Recreate the reference photo shown above',
                referenceImage: 'Reference Photo',
                takePhoto: 'Take Photo',
                retake: 'Retake',
                submit: 'Submit Photo',
                comparison: 'Compare',
                permissionDenied: 'Camera access is required for this task',
                imageLoadFailed: 'Failed to load reference image',
            },
            triviaQuestion: {
                title: 'Trivia Question',
                answerPlaceholder: 'Type your answer here...',
                submit: 'Submit Answer',
                correct: 'Correct!',
                incorrect: 'Incorrect. The correct answer was: {{answer}}',
            },
            custom: {
                title: 'Task',
                instructions: 'Follow the instructions below',
                notePlaceholder: 'Add a note (optional)...',
                markComplete: 'Mark Complete',
            },
            common: {
                submitting: 'Submitting...',
                completed: 'Task Completed!',
                failed: 'Task Failed',
                retry: 'Retry',
                skip: 'Skip Task',
                skipConfirmTitle: 'Skip Task?',
                skipConfirmMessage: 'This task is not yet completed. Are you sure you want to skip?',
                hintRevealed: 'Hint',
                continue: 'Continue to Next Waypoint',
                timeWarning: 'Less than 1 minute remaining!',
            },
        },
        errors: {
            locationPermission: 'Location permission is required for this feature',
            gpsTimeout: 'Unable to get your location. Please try again.',
            networkError: 'Network error. Please check your connection.',
        },
    },
    createQuest: {
        // Main screen
        title: 'Create Quest',
        creating: 'Creating...', 
        createButton: 'Create Quest ({{count}})',

        // BasicInfoForm
        basicInfo: {
            sectionTitle: 'Basic Information',
            title: 'Title',
            titlePlaceholder: 'Enter quest title',
            description: 'Description',
            descriptionPlaceholder: 'Describe your quest',
            reward: 'Reward',
            rewardPlaceholder: "What's the reward?",
        },

        // QuizConfigForm
        quizConfig: {
            loadingConfig: 'Loading configuration...', 
            gameType: 'Game Type',
            wwwQuiz: 'WWW Quiz',
            teamName: 'Team Name',
            teamNameRequired: 'Team Name *',
            teamNamePlaceholder: 'Enter your team name',
            teamMembers: 'Team Members',
            teamMembersRequired: 'Team Members *',
            addMemberPlaceholder: 'Add team member',
            add: 'Add',
            remove: 'Remove',
            difficulty: 'Difficulty',
            easy: 'Easy',
            medium: 'Medium',
            hard: 'Hard',
            timePerQuestion: 'Time per Question',
            roundCount: 'Round Count',
            rounds: '{{count}} rounds',
            enableAIHost: 'Enable AI Host',
            aiHostDescription: 'AI will guide the game and provide commentary',
            enableAiAnswerValidation: 'Accept Similar Answers (AI)',
            aiAnswerValidationDesc: 'AI will accept synonyms and paraphrased answers as correct (e.g. "car" = "automobile")',
            teamBased: 'Team Based',
            teamBasedDescription: 'Players compete in teams',
            configSummary: 'Configuration Summary',
            summaryDifficulty: 'Difficulty: {{value}}',
            summaryTime: 'Time per question: {{value}}s',
            summaryQuestions: 'Total questions: {{count}}',
            summaryAIHost: 'AI Host: {{value}}',
            summaryAiValidation: 'AI Answer Validation: {{value}}',
            summaryTeamBased: 'Team Based: {{value}}',
            yes: 'Yes',
            no: 'No',
            notSet: 'Not set',
            participationSettings: 'Participation Settings',
            openEnrollment: 'Open Enrollment',
            openEnrollmentDesc: 'Allow users to join without an invitation.',
            maxParticipants: 'Max Participants',
            unlimited: 'Unlimited',
            completionSettings: 'Completion Settings',
            shuffleQuestions: 'Shuffle Questions',
            shuffleQuestionsDesc: 'Randomize question order for each participant.',
            maxAttempts: 'Max Attempts Per User',
            resultSharing: 'Result Sharing Settings',
            requireConsent: 'Require Result Consent',
            requireConsentDesc: 'Ask participant before sharing their results.',
        },

        // QuestionSourceSelector
        questionSource: {
            sectionTitle: 'Question Source',
            addQuestion: 'Add Question',
            appQuestions: 'App Questions',
            appQuestionsDesc: 'Curated questions',
            myQuestions: 'My Questions',
            myQuestionsDesc: 'Your created questions',
        },

        // QuestionList
        questionList: {
            searchPlaceholder: 'Search questions...', 
            filterByDifficulty: 'Filter by difficulty',
            filterByTopic: 'Filter by topic',
            allDifficulties: 'All Difficulties',
            allTopics: 'All Topics',
            search: 'Search',
            loading: 'Loading questions...', 
            noQuestions: 'No questions found',
            noQuestionsDesc: 'Try adjusting your search or filters',
            selectAll: 'Select All',
            deselectAll: 'Deselect All',
            selected: '{{count}} selected',
            showAnswer: 'Show Answer',
            hideAnswer: 'Hide Answer',
            expand: 'Expand',
            collapse: 'Collapse',
            page: 'Page {{current}} of {{total}}',
            previous: 'Previous',
            next: 'Next',
            question: 'Question',
            answer: 'Answer',
            topic: 'Topic',
            additionalInfo: 'Additional Info',
            searchTopicsPlaceholder: 'Search topics...',
            noTopicsFound: 'No topics match your search',
            topicCount: '{{count}} topics',
            swipeForMore: 'Swipe for more',
        },

        // SelectedQuestionsPreview
        preview: {
            title: 'Selected Questions',
            collapse: 'Collapse',
            expand: 'Expand',
            noQuestions: 'No questions selected',
            noQuestionsDesc: 'Select questions from the list above',
            questionNumber: 'Question {{number}}',
            fromApp: 'App',
            fromUser: 'Custom',
            remove: 'Remove',
            page: 'Page {{current}} of {{total}}',
        },

        // AddQuestionModal
        addQuestion: {
            title: 'Add Custom Question',
            questionLabel: 'Question *',
            questionPlaceholder: 'Enter your question',
            answerLabel: 'Answer *',
            answerPlaceholder: 'Enter the answer',
            difficultyLabel: 'Difficulty',
            topicLabel: 'Topic (Optional)',
            topicPlaceholder: 'Select or enter topic',
            additionalInfoLabel: 'Additional Info (Optional)',
            additionalInfoPlaceholder: 'Sources, hints or context',
            acceptSimilarAnswers: 'Accept Similar Answers',
            acceptSimilarAnswersDesc: 'AI will accept synonyms and paraphrased answers as correct (e.g. "car" = "automobile")',
            acceptSimilarAnswersAudioNote: 'Audio challenges use audio scoring instead of text matching',
            submit: 'Add Question',

            cancel: 'Cancel',
            errorTitle: 'Error',
            errorQuestion: 'Please enter a question',
            errorAnswer: 'Please enter an answer',
        },

        // QuestionTypeSelectorModal
        typeSelector: {
            title: 'Select Question Type',
            regularQuestion: 'Regular Question',
            regularDescription: 'Create a classic quiz question with optional image, video, or audio.',
            karaokeQuestion: 'Karaoke Question',
            karaokeDescription: 'Create an audio challenge where users sing or match sounds.',
        },

        // RegularQuestionEditor / MediaQuestionModal
        questionEditor: {
            title: 'Create Question',
            editTitle: 'Edit Question',
            mediaPreview: 'Media Preview',
            addMedia: 'Add Media',
            selectImage: 'Select Image',
            selectVideo: 'Select Video',
            selectAudio: 'Select Audio',
            removeMedia: 'Remove Media',
            removeMediaConfirm: 'Are you sure you want to remove this media?',
            discardChanges: 'Discard Changes?',
            discardChangesMessage: 'You have unsaved changes. Are you sure you want to close?',
            discard: 'Discard',
            cancel: 'Cancel',
            remove: 'Remove',
            submit: 'Save Question',
            errorTitle: 'Error',
            errorQuestion: 'Please enter a question',
            errorAnswer: 'Please enter an answer',
            errorTopic: 'Please select or enter a topic',
            errorMedia: 'Failed to select media. Please try again.',
            videoFile: 'Video file selected',
            audioFile: 'Audio file selected',
            tapToPreview: 'Tap to preview',
            videoSource: 'Video Source',
            upload: 'Upload',
            link: 'Link',
            pasteLinkPlaceholder: 'Paste YouTube or video URL...', 
            clearLink: 'Clear Link',
            playbackRange: 'Playback Range (optional)',
            startSec: 'Start (sec)',
            endSec: 'End (sec)',
            invalidFile: 'Invalid File',
            validImage: 'Please select a valid image',
            validVideo: 'Please select a valid video',
            validAudio: 'Please select a valid audio',
            enterVideoUrl: 'Please enter a video URL',
            validVideoUrl: 'Please enter a valid video URL (YouTube, Vimeo, or direct video link)',
            endTimeError: 'End time must be greater than start time',
            selectVideoDevice: 'Select Video from Device',
            mediaSelected: '{{type}} Selected - will upload when saved',
        },

        // Alerts and errors
        alerts: {
            createSuccess: 'Quest created successfully!',
            createError: 'Failed to create quest. Please try again.',
            validationError: 'Please fill in all required fields',
            minQuestions: 'Please select at least {{min}} questions',
            loginRequired: 'Please log in to create a quest',
            titleRequired: 'Please enter a quest title',
            descriptionRequired: 'Please enter a quest description',
        },

        // Wager setup
        wager: {
          title: "Wager Settings",
          addButton: "Add a Wager (Optional)",
          configuredButton: "Wager Set Up ✅"
        },
    },
    settings: {
        title: 'Settings',
        language: 'Language',
        languageDescription: 'Choose your preferred language',
        theme: 'Theme',
        themeDescription: 'Choose your preferred theme',
        themeLight: 'Light',
        themeDark: 'Dark',
        themeSystem: 'System',
        notifications: 'Notifications',
        notificationsDescription: 'Enable push notifications',
        about: 'About',
        version: 'Version',
        privacy: 'Privacy Policy',
        terms: 'Terms of Service',
        quizSettings: 'Quiz Settings',
        aiAnswerValidation: 'Accept Similar Answers (AI)',
        aiAnswerValidationDescription: 'Use AI to accept synonyms and paraphrased answers as correct in quizzes',
        screenTime: {
            title: 'Screen Time Tracking',
            enabled: 'Enable Screen Time Tracking',
            enabledDescription: 'When enabled, your app usage will be limited by your daily budget.',
            controlledBy: 'Controlled by {{username}}',
            controlLocked: 'Only {{username}} can change this setting',
            delegateTitle: 'Delegate Control',
            delegateDescription: 'Give control of your screen time settings to a friend for accountability.',
            delegateButton: 'Delegate Control to Someone',
            delegateConfirmTitle: 'Are you sure?',
            delegateConfirmMessage: 'Once delegated, only {{username}} can change your screen time settings. This is designed for accountability.',
            releaseButton: 'Release Control',
            releaseConfirmTitle: 'Release Control?',
            releaseConfirmMessage: 'Are you sure you want to release control of {{username}}\'s screen time settings?',
            selfManaged: 'Self-managed',
        },
        logout: "Logout",
        logoutConfirm: 'Are you sure you want to log out?',
        account: 'Account',
        preferences: 'Preferences',
        support: 'Support & Info',
        updateSuccess: 'Settings updated successfully',
        updateError: 'Failed to update settings',
        themes: {
            light: 'Light',
            dark: 'Dark',
            system: 'System',
        },
        gender: 'Gender',
        genderDescription: 'For gender-specific invitations',
        genderOptions: {
            MALE: 'Male',
            FEMALE: 'Female',
            OTHER: 'Other',
            PREFER_NOT_TO_SAY: 'Prefer Not to Say',
        },
        invitationPreferences: {
            title: 'Invitation Privacy',
            allowInvitationsFrom: 'Allow invitations from:',
            genderPreference: 'Gender preference:',
            saveChanges: 'Save Changes',
            updateSuccess: 'Preferences updated',
            updateError: 'Failed to update preferences',
            options: {
                ANYONE: 'Anyone',
                FRIENDS_ONLY: 'Friends Only',
                FAMILY_ONLY: 'Family Only',
                FRIENDS_AND_FAMILY: 'Friends & Family',
                NOBODY: 'Nobody',
                ANY_GENDER: 'Any Gender',
                MALE_ONLY: 'Male Only',
                FEMALE_ONLY: 'Female Only',
            },
        },
    },
    navigation: {
        home: 'Home',
        challenges: 'Challenges',
        search: 'Search',
        contacts: 'Contacts',
        groups: 'Groups',
        profile: 'Profile',
        settings: 'Settings',
        createAudioQuestion: 'Create Audio Question',
    },
    auth: {
        login: 'Login',
        logout: 'Logout',
        signup: 'Sign Up',
        username: 'Username',
        password: 'Password',
        email: 'Email',
        forgotPassword: 'Forgot Password?',
        noAccount: "Don't have an account?",
        hasAccount: 'Already have an account?',
        loginButton: 'Sign In',
        signupButton: 'Create Account',
        loginSuccess: 'Login successful',
        loginFailed: 'Login failed',
        invalidCredentials: 'Invalid credentials',
        fillAllFields: 'Please fill in all fields',
        welcomeBack: 'Welcome Back!',
        signInSubtitle: 'Sign in to continue',
        loggingIn: 'Logging in...', 
        createAccount: 'Create Account',
        chooseUsername: 'Choose a username',
        enterEmail: 'Enter your email',
        enterPassword: 'Enter your password',
        confirmPassword: 'Confirm Password',
        confirmPasswordPlaceholder: 'Confirm your password',
        creatingAccount: 'Creating Account...', 
        signupFailed: 'Signup Failed',
        passwordsDoNotMatch: 'Passwords do not match',
        invalidEmail: 'Please enter a valid email address',
        passwordTooShort: 'Password must be at least 6 characters long',
        usernameOrEmailExists: 'Username or email already exists',
        genericError: 'Signup failed. Please try again.',
        signInLink: 'Sign In',
        sessionExpired: {
            title: 'Session Expired',
            message: 'Your session has expired. Please log in again.',
        },
        authRequired: {
            title: 'Authentication Required',
            message: 'Please log in to continue.',
        },
    },
    home: {
        welcome: 'Welcome',
        greeting: 'Hello',
        recentChallenges: 'Recent Challenges',
        quickActions: 'Quick Actions',
        noRecentChallenges: 'No recent challenges',
        startNewGame: 'Start New Game',
        viewAll: 'View All',
        createQuiz: 'Create Quiz',
        newChallenge: 'New Challenge',
        findChallenges: 'Find Challenges',
        findCompanions: 'Find Companions',
        myProfile: 'My Profile',
        wwwQuizChallenges: 'WWW Quiz Challenges',
        seeAll: 'See All →',
        explore: 'Explore',
    },
    challenges: {
        title: 'Challenges',
        filterLabel: 'Show challenges where I am:',
        participant: 'Participant',
        creator: 'Creator',
        loading: 'Loading challenges...', 
        error: 'Failed to load challenges',
        retry: 'Retry',
        emptyTitle: 'No challenges found',
        emptyJoined: "You haven't joined any challenges yet",
        emptyCreated: "You haven't created any challenges yet",
        emptyAdjust: 'Try adjusting your filters',
        createTitle: 'Create Challenge',
        createMessage: 'Choose the type of challenge you want to create:',
        quizChallenge: 'Quiz Challenge',
        standardChallenge: 'Standard Challenge',
        sortLabel: 'Sort by:',
        sortNewest: 'Newest',
        sortOldest: 'Oldest',
        sortTitleAZ: 'A → Z',
        sortTitleZA: 'Z → A',
        completed: 'Completed',
        emptyCompleted: 'No completed quests yet',
        emptyCompletedHint: 'Complete your first quest to see it here!',
    },
    challengeCard: {
        wager: {
            title: 'Wager',
            points: '{{amount}} pts',
            screenTime: '{{amount}} min',
            money: '{{amount}}',
            socialQuest: 'Social Quest',
        },
        visibility: {
            private: 'Private',
        },
        payment: {
            free: 'FREE',
        },
        description: {
            empty: 'No description provided',
        },
        metadata: {
            type: 'Type',
            participants: 'Participants',
            frequency: 'Frequency',
        },
        prize: {
            label: 'Prize',
            pool: 'Prize Pool',
        },
        creator: {
            createdBy: 'Created by {{username}}',
            yourChallenge: 'Your Challenge',
        },
        currency: {
            pts: '{{amount}} pts',
        },
    },

    quizChallengeCard: {
        type: {
            www: 'What? Where? When?',
            quiz: 'Quiz',
        },
        badge: {
            questions: '{{count}} Questions',
            team: 'Team',
        },
        creator: 'Creator',
        reward: 'Reward: {{reward}}',
        status: {
            active: 'ACTIVE',
            completed: 'COMPLETED',
            failed: 'FAILED',
            open: 'OPEN',
            in_progress: 'IN PROGRESS',
            cancelled: 'CANCELLED',
            expired: 'EXPIRED',
            draft: 'DRAFT',
        },
    },

    challengeSearch: {
        title: 'Search Challenges',
        placeholder: 'Search by title or description...',
        filters: {
            all: 'All',
            free: 'Free',
            paid: 'Paid',
            private: 'Private',
        },
        results: {
            count_one: '{{count}} challenge found',
            count_other: '{{count}} challenges found',
        },
        loading: 'Searching challenges...',
        error: 'Failed to load challenges',
        retry: 'Retry',
        empty: {
            title: 'No challenges found',
            withQuery: 'No challenges match "{{query}}"',
            noQuery: 'Try searching for something else',
        },
    },

    completedQuestCard: {
        typeBadge: 'QUIZ',
        bestScore: 'Best Score',
        played: 'Played {{count}} times',
        lastPlayed: 'Last: {{date}}',
        never: 'Never',
    },

    challengeSearchCard: {
        quizFallback: 'Quiz Challenge',
        challengeFallback: 'Challenge',
    },
    search: {
        placeholder: 'Search users, quizzes, challenges...', 
        loading: 'Searching...', 
        title: 'Search WWW Quest',
        subtitle: 'Find users, quizzes, and challenges',
        recent: 'Recent Searches',
        minChars: 'Type at least 2 characters to search',
        noResults: 'No results found',
        tryDifferent: 'Try a different search term',
        foundResults: 'Found {{count}} result{{plural}} for "{{query}}" ',
        users: 'Users',
        quizzes: 'Quizzes',
        challenges: 'Challenges',
        seeAll: 'See all {{count}} {{type}}',
        connected: 'Connected',
        pending: 'Pending',
    },
    game: {
        wwwTitle: 'What? Where? When?',
        startGame: 'Start Game',
        nextQuestion: 'Next Question',
        submitAnswer: 'Submit Answer',
        timeRemaining: 'Time Remaining',
        score: 'Score',
        round: 'Round',
        correct: 'Correct!',
        incorrect: 'Incorrect',
        gameOver: 'Game Over',
        finalScore: 'Final Score',
        playAgain: 'Play Again',
        question: 'Question',
        answer: 'Answer',
        discussion: 'Discussion',
        teamName: 'Team Name',
        difficulty: 'Difficulty',
        aiAcceptedBadge: 'Accepted as equivalent (AI)',
        aiAcceptedExplanation: 'Your answer "{{userAnswer}}" was accepted as equivalent to "{{correctAnswer}}" ',
    },
    profile: {
        editProfile: 'Edit Profile',
        myQuestions: 'My Questions',
        statistics: 'Statistics',
        achievements: 'Achievements',
        created: 'Created',
        completed: 'Completed',
        winRate: 'Win Rate',
        addToContacts: 'Add to Contacts',
        requestPending: 'Request Pending',
        mutualConnections: 'Mutual Connections',
        stats: 'Stats',
        hideCancelled: 'Hide Cancelled/Completed',
        showAll: 'Show All',
        loadingCreated: 'Loading created challenges...', 
        loadingJoined: 'Loading joined challenges...', 
        noCreated: 'No challenges created',
        noJoined: 'No challenges joined',
        noCreatedSelf: "You haven't created any challenges yet",
        noJoinedSelf: "You haven't joined any challenges yet",
        loadingProfile: 'Loading profile...', 
        errorProfile: 'Failed to load user profile.',
        tryAgain: 'Try Again',
        joinedDate: 'Joined {{date}}',
        createdTab: 'Created',
        joinedTab: 'Joined',
    },
    questionDisplay: {
        types: {
            question: 'Question',
            text: 'Text Question',
            image: 'Image Question',
            video: 'Video Question',
            audioChallenge: 'Audio Challenge',
            audio: 'Audio Question',
        },
        karaoke: {
            passScore: 'Pass score: {{percentage}}%',
            completeChallenge: 'Complete the audio challenge',
            listenReference: 'Listen to Reference',
            recordResponse: 'Record Your Response',
        },
    },
    questions: {
        title: 'Questions',
        myQuestions: 'My Questions',
        createQuestion: 'Create Question',
        editQuestion: 'Edit Question',
        deleteQuestion: 'Delete Question',
        deleteConfirm: 'Are you sure you want to delete this question?',
        questionText: 'Question Text',
        answerText: 'Answer',
        topic: 'Topic',
        difficulty: 'Difficulty',
        additionalInfo: 'Additional Info',
        source: 'Source',
        visibility: 'Visibility',
        noQuestions: 'You have no questions yet',
        questionType: 'Question Type',
        text: 'Text',
        image: 'Image',
        video: 'Video',
        audioType: 'Audio',
    },
    validation: {
        required: 'Required field',
        invalidFormat: 'Invalid format',
        tooShort: 'Too short',
        tooLong: 'Too long',
        invalidEmail: 'Invalid email',
        passwordMismatch: 'Passwords do not match',
        minLength: 'Minimum {{count}} characters',
        maxLength: 'Maximum {{count}} characters',
    },
    errors: {
        networkError: 'Network error. Please check your connection.',
        serverError: 'Server error. Please try again later.',
        unknownError: 'An unknown error occurred.',
        sessionExpired: 'Session expired. Please login again.',
    },
    appUpdate: {
        updateAvailable: 'Update Available',
        newVersion: 'Version {{version}} is available',
        currentVersion: 'Current version: {{version}}',
        releaseNotes: "What's New",
        updateNow: 'Update Now',
        later: 'Later',
        downloading: 'Downloading update...', 
        downloadComplete: 'Download Complete',
        install: 'Install Update',
        installing: 'Opening installer...', 
        downloadFailed: 'Download failed. Please try again.',
        checkForUpdates: 'Check for Updates',
        upToDate: "You're up to date!",
        forceUpdateTitle: 'Update Required',
        forceUpdateMessage: 'This update is required to continue using the app.',
        retry: 'Retry',
        downloadProgress: '{{percentage}}% ({{downloaded}} / {{total}})',
        checking: 'Checking for updates...', 
        errorChecking: 'Could not check for updates. Please try again later.',
    },
    userQuestions: {
        // Screen titles
        createTitle: 'Create New Question',
        editTitle: 'Edit Question',
        listTitle: 'My Questions',

        // Form labels
        questionLabel: 'Question',
        questionRequired: 'Question *',
        questionPlaceholder: 'Enter your question',

        answerLabel: 'Answer',
        answerRequired: 'Answer *',
        answerPlaceholder: 'Enter the answer',

        difficultyLabel: 'Difficulty',
        difficultyRequired: 'Difficulty *',

        visibilityLabel: 'Who Can See This?',
        visibilityRequired: 'Who Can See This? *',

        topicLabel: 'Topic (Optional)',
        topicPlaceholder: 'Select or create a topic...',

        additionalInfoLabel: 'Additional Info (Optional)',
        additionalInfoPlaceholder: 'Add sources, hints, or extra context',

        // Buttons
        create: 'Create',
        update: 'Update',
        creating: 'Creating...', 
        updating: 'Updating...', 

        // Alerts & Messages
        questionRequiredError: 'Please enter a question',
        answerRequiredError: 'Please enter an answer',
        createSuccess: 'Question created successfully',
        updateSuccess: 'Question updated successfully',
        saveFailed: 'Failed to save question. Please try again.',
        loadFailed: 'Failed to load your questions',

        // Delete confirmation
        deleteTitle: 'Delete Question',
        deleteConfirm: 'Are you sure you want to delete this question?',
        deleteSuccess: 'Question deleted successfully',
        deleteFailed: 'Failed to delete question',
    },
    editProfile: {
        title: 'Edit Profile',
        save: 'Save',
        saving: 'Saving...', 
        changePhoto: 'Tap to change profile picture',
        selectPicture: 'Select Profile Picture',
        chooseSource: 'Choose from where you want to select a profile picture',
        camera: 'Camera',
        gallery: 'Gallery',
        cancel: 'Cancel',

        // Form fields
        usernameLabel: 'Username',
        usernameRequired: 'Username *',
        usernamePlaceholder: 'Enter your username',
        usernameHelper: 'Minimum 3 characters, maximum 50',

        bioLabel: 'Bio',
        bioPlaceholder: 'Tell something about yourself...', 

        // Messages
        updateSuccess: 'Profile updated successfully',
        updateFailed: 'Failed to update profile',
        permissionDenied: 'Permission denied',
        cameraPermissionMessage: 'Camera permission is required to take photos',
    },
    mediaQuestion: {
        // Question types
        questionTypeLabel: 'Question Type',
        textQuestion: 'Text Question',
        imageQuestion: 'Image Question',
        videoQuestion: 'Video Question',
        audioChallenge: 'Audio Challenge',

        // Form fields
        questionLabel: 'Question',
        questionRequired: 'Question *',
        questionPlaceholder: 'Enter your question here...', 

        answerLabel: 'Answer',
        answerRequired: 'Question *',
        answerPlaceholder: 'Enter the answer...',
        acceptSimilarAnswers: 'Accept Similar Answers',
        acceptSimilarAnswersDesc: 'AI will accept synonyms and paraphrased answers as correct (e.g. "car" = "automobile")',
        acceptSimilarAnswersAudioNote: 'Audio challenges use audio scoring instead of text matching',

        // Media

        selectMedia: 'Select Media',
        uploadImage: 'Upload Image',
        uploadVideo: 'Upload Video',
        uploadAudio: 'Upload Audio',
        removeMedia: 'Remove',

        // Visibility
        visibilityLabel: 'Visibility',
    },
    audioQuestion: {
        title: 'Create Audio Question',

        // Challenge types
        challengeTypeLabel: 'Challenge Type',
        selectChallengeType: 'Select a challenge type',

        // Form fields
        questionLabel: 'Question / Instructions',
        questionPlaceholder: 'Describe what the player should do...', 

        answerHelperText: 'Optional: describe the expected response. For audio challenges, the audio itself serves as the answer.',

        referenceAudio: 'Reference Audio',
        uploadReference: 'Upload Reference Audio',
        recordReference: 'Record Reference',

        // Sliders
        bpmLabel: 'BPM (Beats Per Minute)',
        timeSignatureLabel: 'Time Signature',
        minimumScoreLabel: 'Minimum Score to Pass',

        segmentLabel: 'Audio Segment',
        startTime: 'Start Time',
        endTime: 'End Time',

        // Buttons
        create: 'Create Audio Question',
        creating: 'Creating...', 

        responseMethod: 'Response Method',
        responseMethodDesc: 'How players will respond to this challenge',
        trimAudio: 'Trim Audio',
        trimAudioDesc: 'Select the portion of audio to use as the challenge',
        segmentStart: 'Start',
        segmentEnd: 'End',
        segmentDuration: 'Selected: {{duration}}',
        totalDuration: 'Total: {{duration}}',
        previewAudio: 'Preview',
        tapToPreview: 'Tap to preview your recording',

        // Messages
        createSuccess: 'Audio question created successfully',
        createFailed: 'Failed to create audio question. Please try again.',
        playbackError: 'Failed to play audio. The file might be corrupted or moved.',
        audioRequired: 'Reference audio is required for this challenge type',
        recordReferenceAudio: 'Record Reference Audio',
        answerMode: {
            title: 'Answer Input Mode',
            tap: 'Tap Screen',
            tapDescription: 'Player taps the screen to match the rhythm',
            audio: 'Record Audio',
            audioDescription: 'Player records claps or sounds via microphone',
            both: "Player's Choice",
            bothDescription: 'Player can choose either mode',
        },
        replay: {
            title: 'Replay Settings',
            allowReplay: 'Allow Replay',
            allowReplayDescription: 'Let players replay the reference pattern before answering',
            maxReplays: 'Maximum Replays',
            unlimited: 'Unlimited',
            replaysCount_one: '{{count}} replay',
            replaysCount_other: '{{count}} replays',
        },
    },
    rhythmChallenge: {
        title: 'Rhythm Challenge',
        attempt: 'Attempt {{count}}',
        listen: 'Listen',
        listenCount: 'Listen ({{count}})',
        listenToPatternFirst: 'Listen to the pattern first!',
        startTapping: 'Start Tapping',
        startRecording: 'Start Recording',
        playingPattern: 'Playing pattern...', 
        getReady: 'Get ready!',
        loadingChallenge: 'Loading challenge...', 
        questionNotFound: 'Question not found',
        processing: 'Processing...', 
        tapMode: 'Tap Mode',
        clapMode: 'Clap Mode',
        patternInfo: '{{beats}} beats • ~{{bpm}} BPM • {{timeSignature}}',
        results: {
            perfect: 'Perfect',
            good: 'Good',
            earlyLate: 'Early/Late',
            missed: 'Missed',
            yourTiming: 'Your timing',
            score: 'Score',
            combined: 'Combined',
            passed: 'PASSED!',
            tryAgain: 'TRY AGAIN',
            scoreBreakdown: 'Score Breakdown',
            timing: 'Timing',
            sound: 'Sound',
            timingPerformance: 'Timing Performance',
            avgError: 'Avg Error',
            soundQualityPerBeat: 'Sound Quality per Beat',
            beat: 'Beat {{number}}',
            yourSound: 'Your sound:',
            timbre: 'Timbre',
            brightness: 'Brightness',
            energy: 'Energy',
            continue: 'Continue',
        },
        soundSimilarity: {
            title: 'Sound Similarity',
            enabledDesc: 'Evaluate both timing AND sound quality',
            disabledDesc: 'Evaluate timing only',
            explanation: 'Your claps will be analyzed for clarity, brightness, and consistency. Try to match the sound quality of the reference pattern.',
            timing: 'Timing',
            sound: 'Sound',
        },
        stopRecording: 'Stop',
        listenAgain: 'Listen Again',
        replaysLeft: '{{count}} replays left',
        replaysLeft_one: '{{count}} replay left',
        replaysLeft_other: '{{count}} replays left',
        unlimitedReplays: 'Unlimited replays',
        noReplaysLeft: 'No replays left',
        tapAlong: 'Tap along with the rhythm!',
        recording: 'Recording...', 
        notEnoughTaps: 'Not enough taps',
        notEnoughTapsMessage: 'Please tap at least 2 times to submit.',
        retry: 'Try Again',
        continue: 'Continue',
        done: 'Done',
        tapsProgress: '{{current}} / {{total}} taps',
        referenceRhythm: 'Reference rhythm',
        yourTaps: 'Your taps',
        tapHere: 'Tap here',
        tapsLabel: 'taps',
        tapTheRhythm: 'Tap to the rhythm!',
        onTime: 'On time',
        early: 'Early',
        late: 'Late',
        listenFirst: 'Listen to the pattern first',
        analyzingSound: 'Analyzing sound quality...',
        analyzingSoundDesc: 'Your timing results are ready. Sound analysis takes a moment.',
        soundAnalysisUnavailable: 'Sound analysis unavailable',
        timingScore: 'Timing Score',
        soundScore: 'Sound Score',
        combinedScore: 'Combined Score',
        phaseTimingReady: 'Timing results ready!',
        overallScore: 'Overall Score',
        perfect: 'Perfect',
        good: 'Good',
        missed: 'Missed',
        avgError: 'Avg Error',
        modeSelector: {
            title: 'Choose your input mode',
            tap: 'Tap',
            audio: 'Audio',
        },
        errors: {
            microphonePermissionTitle: 'Microphone Permission',
            microphonePermission: 'Microphone permission is required to record audio',
            recordingTooShort: 'Recording too short. Please record for at least 1 second.',
            recordingFailed: 'Failed to save recording. Please try again.',
            fileTooLarge: 'Recording file is too large. Please try a shorter recording.',
            invalidAudio: 'Audio format not recognized. Please try again.',
            networkError: 'Network error. Please check your connection and try again.',
            scoringFailed: 'Failed to analyze your rhythm. Please try again.',
            retryFailed: 'Still unable to connect. Please try again later.',
        },
    },
    challenge: {
        // Payment options
        paymentOptions: 'Payment Options',
        paymentType: 'Payment Type',
        free: 'Free - No payment required',
        entryFee: 'Entry Fee - Fixed fee to join',
        prizePool: 'Prize Pool - Fees create prize pool',

        requireEntryFee: 'Require Entry Fee',
        amount: 'Amount',
        currency: 'Currency',
    },
    vibrationQuiz: {
        title: 'Feel the Beat',
        subtitle: 'Guess the song from vibrations',
        feelTheRhythm: 'Feel the Rhythm',
        guessTheSong: 'What song is this?',
        replay: 'Replay',
        replaysLeft: '{{count}} replays left',
        noReplaysLeft: 'No replays left',
        correct: 'Correct!',
        incorrect: 'Not quite...', 
        theAnswerWas: 'The song was',
        nextQuestion: 'Next Question',
        finalScore: 'Final Score',
        playAgain: 'Play Again',
    },
    wager: {
        setup: {
            title: 'Setup Your Wager',
            stakeTypes: {
                POINTS: 'Points',
                SCREEN_TIME: 'Time',
                MONEY: 'Money',
                SOCIAL_QUEST: 'Quest',
            },
            pointsAmount: 'Points Amount',
            pointsAmountPlaceholder: 'e.g. 100',
            moneyAmount: 'Amount',
            moneyAmountPlaceholder: 'e.g. 5.00',
            selectStakeType: 'Select Stake Type',
            stakeAmount: 'Stake Amount',
            stakeAmountPlaceholder: 'e.g. 100',
            screenTime: 'Screen Time (Minutes)',
            screenTimePlaceholder: 'e.g. 30',
            socialPenalty: 'Social Penalty Description',
            socialPenaltyPlaceholder: 'e.g. Change profile pic to a clown for 24h',
            confirmButton: 'Confirm Wager',
            counterOffer: {
                title: 'Propose Counter Offer',
                amountLabel: 'Amount',
                messageLabel: 'Message (Optional)',
                messagePlaceholder: 'Add a note...',
                cancelButton: 'Cancel',
                submitButton: 'Submit Proposal',
            },
        },
        invitation: {
            challengeFrom: 'Wager Challenge from {{username}}',
            expiresIn: 'Expires in: {{time}}',
            stake: 'Stake: {{amount}} {{unit}}',
            points: 'Points',
            minutes: 'Minutes',
            loserPenalty: "Loser's Penalty:",
            decline: 'Decline',
            accept: 'Accept',
            expired: 'Expired',
        },
    },
    puzzle: {
        setup: {
            title: 'Create Puzzle Game',
            selectImage: 'Select Image',
            gridSize: 'Grid Size',
            gameMode: 'Game Mode',
            shared: 'Team Puzzle',
            individual: 'Solo Puzzle',
            answer: 'Correct Answer',
            answerPlaceholder: 'What is in the picture?',
            createGame: 'Create Puzzle',
            generating: 'Cutting puzzle pieces...', 
        },
        game: {
            title: 'Picture Puzzle',
            piecesPlaced: '{{placed}}/{{total}} pieces',
            teamProgress: 'Team Progress',
            hint: 'Show Hint',
            submitAnswer: 'I Know What It Is!',
            answerPlaceholder: 'Type your answer...', 
            submit: 'Submit',
            correct: 'Correct! 🎉',
            incorrect: 'Not quite, keep trying!',
            timeUp: "Time's up!",
            waitingForHost: 'Waiting for host to start...', 
            startGame: 'Start Game',
        },
        results: {
            title: 'Puzzle Results',
            revealed: 'The answer was:',
            yourScore: 'Your Score',
            leaderboard: 'Leaderboard',
            playAgain: 'Play Again',
            backToChallenge: 'Back to Challenge',
            moves: '{{count}} moves',
            time: '{{time}}s',
        },
    },
    challengeVerification: {
        // Main screen
        loading: 'Loading challenge verification...', 
        loadFailed: 'Failed to load challenge details.',
        verificationRequirements: 'Verification Requirements',
        processing: 'Processing verification...', 
        challengeDescription: 'Challenge Description',
        verificationMethods: 'Verification Methods',
        noMethodsFound: 'No verification methods found for this challenge.',
        submitVerification: 'Submit Verification',

        // Status
        status: {
            verified: 'Verified',
            failed: 'Failed',
            pending: 'Pending',
        },

        // Photo verification
        photo: {
            title: 'Photo Verification',
            verified: 'Verified ✓',
            retake: 'Retake Photo',
            take: 'Take Photo',
            defaultPrompt: 'Take a photo for verification',
            instructions: 'Instructions:',
            defaultInstructions: 'Take a clear photo that shows you completing the challenge.',
            loadingChallenge: 'Loading challenge details...', 
            verificationSuccessful: 'Verification Successful',
            successMessage: 'Your photo has been verified successfully!',
            failedMessage: 'Photo verification failed. Please try again.',
            submitFailed: 'Failed to verify photo. Please try again.',
            backToChallenge: 'Back to Challenge',
            takePhotoFirst: 'Please take a photo first.',
            noPhotoTaken: 'No photo taken yet',
            processingError: 'Error: Could not process verification request.',
        },

        // Location verification
        location: {
            title: 'Location Verification',
            verified: 'Verified ✓',
            retry: 'Retry Verification',
            verify: 'Verify Location',
            verifyAtPrompt: "Verify you're at: {{locationName}}",
            requiredLocation: 'required location',
            currentAddress: 'Current: {{address}}',
            distanceInfo: 'Distance: {{distance}}m (Required: within {{radius}}m)',
            instructions: 'Instructions:',
            defaultInstructions: 'You need to be physically present at the challenge location to verify your participation.',
            locationPrefix: 'Location: {{address}}',
            radiusInfo: 'You must be within {{radius}} meters of the target location.',
            yourLocation: 'Your Current Location:',
            targetLocation: 'Target Location:',
            latitude: 'Latitude',
            longitude: 'Longitude',
            address: 'Address',
            notChecked: 'Location not checked yet',
            checkLocation: 'Check My Location',
            checking: 'Checking...', 
            permissionRequired: 'Permission Required',
            permissionMessage: 'Location permissions are required for verification.',
            openSettings: 'Open Settings',
            locationFailed: 'Failed to get your location. Please try again.',
            loadingChallenge: 'Loading challenge details...', 
            verificationSuccessful: 'Verification Successful',
            successMessage: 'Your location has been verified successfully!',
            failedMessage: 'Location verification failed. You are not at the required location.',
            submitFailed: 'Failed to verify location. Please try again.',
            backToChallenge: 'Back to Challenge',
            checkFirst: 'Please check your location first.',
            targetMissing: 'Target location information is missing.',
            processingError: 'Error: Could not process verification request.',
            distanceToTarget: 'Distance to target: {{distance}} meters',
            withinRange: ' (Within range)',
            requiredRadius: ' (Required: {{radius}} meters)',
        },

        // Generic verification types
        types: {
            quiz: 'Quiz',
            manual: 'Manual',
            fitness: 'Fitness',
            activity: 'Activity',
            photo: 'Photo',
            location: 'Location',
        },
        genericTitle: '{{type}} Verification',
        genericDefaultPrompt: 'Complete this verification to finish the challenge.',

        // ChallengeService display names
        displayNames: {
            photo: 'Photo Verification',
            locationCheckin: 'Location Check-in',
            quiz: 'Quiz',
            manual: 'Manual Verification',
            fitnessTracker: 'Fitness Tracker',
            activityTracking: 'Activity Tracking',
        },

        // Time remaining
        timeRemaining: {
            expired: 'Expired',
            noEndDate: 'No end date',
            monthLeft: '{{count}} month left',
            monthLeft_plural: '{{count}} months left',
            daysLeft: '{{count}} day left',
            daysLeft_plural: '{{count}} days left',
            hoursLeft: '{{count}} hour left',
            hoursLeft_plural: '{{count}} hours left',
        },
    },
    createChallenge: {
        title: 'Create Challenge',
        basicInfo: {
            title: 'Title *',
            titlePlaceholder: 'Enter challenge title',
            description: 'Description *',
            descriptionPlaceholder: 'Describe your challenge',
            type: 'Challenge Type',
            types: {
                QUEST: 'Quest',
                QUIZ: 'Quiz',
                ACTIVITY_PARTNER: 'Activity Partner',
                FITNESS_TRACKING: 'Fitness Tracking',
                HABIT_BUILDING: 'Habit Building',
            },
            visibility: 'Visibility',
            visibilityOptions: {
                PUBLIC: 'Public',
                PRIVATE: 'Private',
                GROUP_ONLY: 'Group Only',
            },
            frequency: 'Frequency',
            frequencyOptions: {
                ONE_TIME: 'One Time',
                DAILY: 'Daily',
                WEEKLY: 'Weekly',
            },
        },
        advanced: {
            show: 'Show Advanced Options',
            hide: 'Hide Advanced Options',
            title: 'Advanced Options',
            targetGroup: 'Target Group',
            targetGroupPlaceholder: 'Choose group (optional)',
            tags: 'Tags',
            tagsPlaceholder: 'Enter tags separated by commas (e.g., fitness, daily, workout)',
            reward: 'Reward',
            rewardPlaceholder: "What's the reward for completing this challenge?",
            penalty: 'Penalty',
            penaltyPlaceholder: "What's the penalty for failing?",
        },
        validation: {
            titleRequired: 'Please enter a challenge title',
            descriptionRequired: 'Please enter a challenge description',
            photoDescRequired: 'Please provide a description for photo verification',
            locationRequired: 'Please set location coordinates for location verification',
        },
    },
    gameResults: {
        // --- EXISTING (keep as-is) ---
        alerts: {
            error: {
                title: 'Error',
                submitFailed: 'Failed to submit challenge completion. Please try again.',
            },
            completeChallenge: {
                title: 'Complete Challenge?',
                message: 'Would you like to mark this challenge as completed?',
                yesComplete: 'Yes, Complete It',
                noPlayAgain: 'No, Play Again',
                noReturnHome: 'No, Return Home',
                yes: 'Yes',
                no: 'No',
            },
        },
        completionNotes: 'Quiz completed with score {{score}}/{{total}}',

        screen: {
            score: 'Score: {{score}}/{{total}}',
            correctPercentage: '{{percentage}}% Correct',
            correct: 'CORRECT',
            incorrect: 'INCORRECT',
            questionNumber: 'Question {{number}}',
            yourAnswer: 'Your Answer:',
            correctAnswer: 'Correct Answer:',
            answeredBy: 'Answered by: {{player}}',
            playAgain: 'Play Again →',
            playAgainHint: 'View stats & adjust settings before replaying',
            returnHome: 'Return to Home',
            gameOver: 'Game Over!',
            teamScored: 'Your team scored {{score}} out of {{total}} questions.',
            endGame: 'End Game',
            aiAnalysis: 'AI Analysis',
            playerPerformance: 'Player Performance',
            questionResults: 'Question Results',
            requirementMet: 'Quest Requirement Met',
            requirementNotMet: 'Quest Requirement Not Met',
            requirementDescription: 'Minimum score required: {{required}}% | Your score: {{actual}}%',
            requirementHelp: 'You need to score at least {{required}}% to complete this quest. Try again!',
        },

        // --- NEW KEYS BELOW ---
        score: {
            label: 'Score: {{score}}/{{total}}',
            percentCorrect: '{{percent}}% Correct',
        },
        requirement: {
            met: 'Quest Requirement Met',
            notMet: 'Quest Requirement Not Met',
            description: 'Minimum score required: {{required}}% | Your score: {{actual}}%',
            help: 'You need to score at least {{required}}% to complete this quest. Try again!',
        },
        sections: {
            aiAnalysis: 'AI Host Analysis',
            playerPerformance: 'Player Performance',
            questionResults: 'Question Results',
        },
        question: {
            number: 'Question {{number}}',
            correct: 'CORRECT',
            incorrect: 'INCORRECT',
            yourAnswer: 'Your Answer:',
            correctAnswer: 'Correct Answer:',
            answeredBy: 'Answered by: {{player}}',
        },
        buttons: {
            playAgain: 'Play Again',
            returnHome: 'Return to Home',
        },
        modal: {
            title: 'Game Over!',
            teamScored: 'Your team scored {{score}} out of {{total}} questions.',
            perfectScore: 'Perfect score! Incredible!',
            wellDone: 'Well done!',
            betterLuck: 'Better luck next time!',
            seeResults: 'See Detailed Results',
        },
    },
    friendsFamily: {
        header: {
            title: 'Friends & Family',
            subtitle: 'Manage your connections to share questions',
        },
        addSection: {
            title: 'Add New Connection',
            friend: 'Friend',
            family: 'Family',
            placeholder: 'Enter user ID to connect',
            send: 'Send',
            tip: 'Users you add as friends or family can access questions you mark as "Friends & Family"',
        },
        tabs: {
            connections: 'Connections ({{count}})',
            pending: 'Pending ({{count}})',
        },
        emptyStates: {
            noConnections: {
                title: 'No Connections Yet',
                text: 'Add friends and family to share your custom questions with them',
            },
            noPending: {
                title: 'No Pending Requests',
                text: "You'll see connection requests from other users here",
            },
        },
        alerts: {
            error: {
                userIdRequired: 'Please enter a user ID',
                failedToSend: 'Failed to send request',
                failedToAccept: 'Failed to accept request',
                failedToReject: 'Failed to reject request',
                failedToRemove: 'Failed to remove connection',
            },
            success: {
                requestSent: 'Connection request sent!',
                accepted: 'Connection accepted!',
                rejected: 'Request rejected',
                removed: 'Connection removed',
            },
            reject: {
                title: 'Reject Request',
                message: 'Are you sure you want to reject this request?',
                confirm: 'Reject',
            },
            remove: {
                title: 'Remove Connection',
                message: 'Remove {{username}} from your connections?',
                confirm: 'Remove',
            },
        },
        card: {
            friend: 'Friend',
            family: 'Family',
            wantsToConnect: 'wants to connect as {{type}}',
        },
    },
    contacts: {
        header: {
            title: 'Contacts',
        },
        tabs: {
            contacts: 'Contacts',
            requests: 'Requests',
        },
        search: {
            placeholder: 'Search contacts...', 
        },
        categories: {
            all: 'All',
            friends: 'Friends',
            family: 'Family',
            work: 'Work',
            favorites: 'Favorites',
        },
        emptyStates: {
            noContacts: 'No contacts found',
            noRequests: 'No pending requests',
        },
        alerts: {
            error: {
                favoriteUpdate: 'Failed to update favorite status',
                failedToAccept: 'Failed to accept request',
                failedToReject: 'Failed to reject request',
                failedToRemove: 'Failed to remove contact',
            },
            remove: {
                title: 'Remove Contact',
                message: 'Are you sure you want to remove {{name}}?',
                confirm: 'Remove',
            },
            reject: {
                title: 'Reject Request',
                message: 'Are you sure you want to reject this request?',
                confirm: 'Reject',
            },
        },
        requestText: 'wants to connect as {{type}}',
    },
    groups: {
        header: {
            title: 'Groups',
        },
        search: {
            placeholder: 'Search groups...', 
        },
        filters: {
            label: 'Filter by:',
            types: {
                all: 'All',
                challenges: 'Challenges',
                social: 'Social',
            },
            roles: {
                all: 'All Roles',
                admin: 'My Groups',
                member: 'Joined',
            },
        },
        loading: 'Loading groups...', 
        error: {
            failedToLoad: 'Failed to load groups.',
            retry: 'Retry',
        },
        emptyStates: {
            noMatch: 'No groups match your filters',
            noGroups: "You haven't joined any groups yet",
            createButton: 'Create a Group',
        },
        card: {
            members: '{{count}} members',
            created: 'Created: {{date}}',
        },
        alerts: {
            success: {
                joined: 'You have joined this group!',
            },
            error: {
                joinFailed: 'Failed to join group. Please try again.',
            },
            details: {
                title: 'Group Details',
                message: 'Navigating to details for group {{id}}',
            },
            create: {
                title: 'Create Group',
                message: 'This would navigate to group creation screen',
            },
        },
    },
    controllerGame: {
        phases: {
            listen: {
                title: 'Listen Carefully!',
                subtitle: 'Discussion in progress...', 
                buzz: 'BUZZ!',
            },
            answering: {
                sent: 'Answer Sent ✓',
                wait: 'Waiting for others...', 
                placeholder: 'Type your answer...', 
                submit: 'SUBMIT',
            },
            feedback: {
                title: 'Round Finished',
                subtitle: 'Look at the TV!',
            },
            wait: 'Waiting for next phase...', 
        },
        reconnecting: 'Reconnecting...', 
        score: {
            pts: '{{count}} pts',
        },
    },
    joinRoom: {
        title: 'Join Room',
        subtitle: 'Enter the 6-character code shown on the TV',
        placeholder: 'ABC123',
        joinButton: 'JOIN GAME',
        scanButton: 'SCAN QR CODE',
    },
    challengeDetails: {
        actions: {
            invitePlayers: 'Invite Players',
            startQuiz: 'Start Quiz',
            replayQuiz: 'Replay Quiz',
            joinChallenge: 'Join Challenge',
            submitProof: 'Submit Proof',
            markAsComplete: 'Mark as Complete',
            proofSubmitted: 'Proof Submitted ✓',
            joining: 'Joining...', 
            starting: 'Starting...', 
            submitting: 'Submitting...', 
        },
        info: {
            title: 'Challenge Info',
            created: 'Created',
            visibility: 'Visibility',
            reward: 'Reward',
            penalty: 'Penalty',
            verification: 'Verification',
            unknown: 'Unknown',
            group: 'Group',
        },
        creator: {
            title: 'Created By',
            userId: 'User ID: {{id}}',
            unknown: 'Unknown Creator',
        },
        quiz: {
            title: 'Quiz Challenge',
            gameType: 'Game Type',
            difficulty: 'Difficulty',
            rounds: 'Rounds',
            timePerRound: 'Time per Round',
            team: 'Team',
            standardQuiz: 'Standard Quiz',
            quiz: 'Quiz',
            medium: 'Medium',
        },
        description: {
            title: 'Description',
            empty: 'No description provided',
        },
        audio: {
            title: 'Quest Audio Track',
            requirement: 'You must score at least {{percentage}}% to complete this quest',
        },
        wager: {
            title: 'Pending Wager',
            amount: 'Wager Amount',
            accept: 'Accept',
            decline: 'Decline',
        },
        debug: {
            title: 'Debug Info',
            isCreator: 'Is Creator:',
            hasJoined: 'Has joined:',
            participants: 'Participants:',
        },
        messages: {
            loading: 'Loading challenge...', 
            error: 'Failed to load challenge',
            idNotFound: 'Challenge ID not found. Returning to challenges list.',
            cancelled: 'This quest has been cancelled',
        },
        launcher: {
            comingSoon: 'Coming Soon',
            typeNotSupported: 'Quiz type "{{type}}" is not yet supported.',
            audioConfigMissing: 'Audio configuration missing for this challenge.',
            audioChallenge: 'Audio Challenge',
            audioChallengeInstructions: 'Listen to the audio track and complete the verification tasks.',
            failedToStart: 'Failed to start quiz session. Please try again.',
        },
        status: {
            active: 'Active',
            cancelled: 'Cancelled',
            completed: 'Completed',
            expired: 'Expired',
            failed: 'Failed',
        },
        gameTypes: {
            www: 'What? Where? When?',
            blitz: 'Blitz',
            trivia: 'Trivia',
            custom: 'Custom',
            audio: 'Audio Challenge',
            quiz: 'Quiz',
            quest: 'Quest',
            activity_partner: 'Activity Partner',
            fitness_tracking: 'Fitness Tracking',
            habit_building: 'Habit Building',
            puzzle: 'Puzzle',
        },
        difficulty: {
            mixed: 'Mixed',
        },
        verification: {
            photo: 'Photo Verification',
            location: 'Location Check-in',
            quiz: 'Quiz',
            manual: 'Manual Verification',
            fitness: 'Fitness Tracker',
            activity: 'Activity Tracking',
        },
        sessionHistory: {
            title: 'Session History',
            sessionNumber: 'Session #{{number}}',
            score: '{{correct}}/{{total}} ({{percentage}}%)',
            durationFormat: '{{minutes}}m {{seconds}}s',
            noDuration: '—',
        },
    },
    audioGamePlay: {
        preparingChallenge: 'Preparing audio challenge...', 
        recording: 'Recording your response...', 
        processing: 'Scoring your performance...', 
        submissionFailed: 'Scoring failed. Would you like to try again?',
        tryAgain: 'Try Again',
        skipQuestion: 'Skip',
        tapToStart: 'Tap to start recording',
        listenFirst: 'Listen to the reference audio first',
        rhythmReady: 'Ready to tap? Start when you\'re ready!',
        rhythmListenFirst: 'Listen to the pattern, then repeat it',
        soundMatchInstructions: 'Match the sounds you hear as closely as possible',
        singingInstructions: 'Sing along with the track',
        minTapsRequired: 'Please tap at least a few times to be scored.',
        listenToPattern: 'Listen to Pattern',
        readyToTap: 'Ready to tap? Start when you\'re ready!',
        relistenDuringTap: 'Re-listen',
        tappingPaused: 'Paused — listen again, then continue',
        relisten: 'Listen Again',
        answerDisabledWhilePlaying: 'Answer will unlock after listening',
        listenToQuestionFirst: 'Listen to the question first',
    },
    games: {
        title: 'Games',
        subtitle: 'Challenge your mind',
        comingSoon: 'COMING SOON',
        comingSoonAlert: 'Coming soon!',
        featuredGame: 'Featured Game',
        playNow: 'PLAY NOW',
        adminTools: {
            title: 'Admin Tools',
            description: 'These tools allow you to manage game content and settings.',
        },
        recentActivity: {
            title: 'Recent Activity',
            noActivity: 'No recent games played. Start a new game to see your activity here!',
        },
        gameHistory: {
            title: 'Game History',
            searchPlaceholder: 'Search by team, player, or title...', 
            noHistory: 'No games played yet',
            noHistorySubtext: 'Start a new game and your history will appear here!',
            noResults: 'No matches found',
            noResultsSubtext: 'Try adjusting your search or filters',
            filters: {
                all: 'All',
                status: 'Status',
                difficulty: 'Difficulty',
                gameMode: 'Game Mode',
            },
            status: {
                completed: 'Completed',
                inProgress: 'In Progress',
                paused: 'Paused',
                abandoned: 'Abandoned',
                cancelled: 'Cancelled',
                created: 'Created',
            },
            gameMode: {
                standard: 'Standard',
                brainRing: 'Brain Ring',
                blitz: 'Blitz',
            },
            difficulty: {
                easy: 'Easy',
                medium: 'Medium',
                hard: 'Hard',
            },
            card: {
                score: '{{correct}}/{{total}}',
                duration: '{{minutes}} min',
                rounds: '{{count}} rounds',
                team: 'Team: {{name}}',
                players: '{{count}} players',
                aiHost: 'AI Host',
                resume: 'Resume',
                viewResults: 'View Results',
            },
            pullToRefresh: 'Pull to refresh',
            loadError: 'Failed to load game history',
            retry: 'Retry',
        },
        www: {
            title: 'WWW Quiz',
            description: 'A team trivia game where players discuss and answer challenging questions.',
            featuredDescription: 'A challenging team-based trivia game with AI-powered hosting',
        },
        vibration: {
            title: 'Feel the Beat',
            description: 'Guess the song from its rhythm vibrations!',
        },
        rhythm: {
            title: 'Rhythm Game',
            description: 'Test your timing with this fun rhythm-based challenge.',
        },
        quiz: {
            title: 'Quiz Challenge',
            description: 'Test your knowledge in various categories.',
        },
        questionManagement: {
            title: 'Question Management',
            description: 'Add, edit, and manage questions for the WWW Quiz game.',
        },
    },
    penalty: {
        title: 'My Penalties',
        tabs: {
            pending: 'Pending',
            active: 'Active',
            completed: 'Completed',
        },
        status: {
            pending: 'Pending',
            inProgress: 'In Progress',
            verified: 'Verified',
            failed: 'Failed',
        },
        due: 'Due: {{date}}',
        noPenalties: 'No penalties in this category!',
        proof: {
            title: 'Submit Proof',
            photoProof: 'Photo Proof',
            tapToSelect: 'Tap to select an image',
            descriptionLabel: 'Description (Optional)',
            descriptionPlaceholder: 'Explain how you completed the penalty...', 
            submitButton: 'Submit Completion',
            errorNoContent: 'Please provide either a photo or a text description.',
            submitSuccess: 'Proof submitted successfully!',
            submitError: 'Failed to submit proof. Please try again.',
        },
    },
    competitive: {
        matchmaking: {
            cancel: 'Cancel',
            lookingForOpponent: 'Looking for opponent...', 
            queuePosition: 'Position in queue: {{position}}',
            estimatedWait: 'Estimated wait: ~{{seconds}}s',
        },
        liveMatch: {
            loading: 'Loading match...', 
            round: 'Round {{current}} / {{total}}',
            opponent: 'Opponent',
            status: 'Status: {{status}}',
            startMatch: 'Start Match',
            challenge: 'Challenge: {{challenge}}',
            audioChallenge: 'Audio Challenge',
            startRecording: 'Start Recording',
            stopAndSubmit: 'Stop & Submit',
            opponentStatus: 'Opponent Status:',
            submitted: 'Submitted ✅',
            recording: 'Recording... 🎙️',
            submissionAlert: 'Submission',
            submissionSuccessMock: 'Recording submitted (mock)',
        },
        matchLobby: {
            loading: 'Loading lobby...', 
            title: 'Match Lobby',
            rounds: '{{count}} Rounds',
            waitingForOpponent: 'Waiting...', 
            waitingDescription: 'Waiting for opponent to accept...', 
            startMatch: 'Start Match',
        },
        matchResult: {
            loading: 'Loading results...', 
            title: 'Match Results',
            draw: 'Draw!',
            winner: 'Winner: {{username}}',
            player1: 'Player 1',
            player2: 'Player 2',
            roundsWon: '{{count}} Rounds',
            wonAmount: 'Won: {{amount}} {{currency}}',
            points: 'Points',
            returnHome: 'Return Home',
        },
        history: {
            title: 'Match History',
            opponent: 'Opponent',
            noMatches: 'No matches found',
        },
    },
    audioChallenge: {
        countdown: {
          getReady: 'Get ready!',
          go: 'GO!',
          starting: 'Starting in...',
        },
        karaoke: {
          backingTrackPlaying: 'Backing track playing',
          backingTrackError: 'Backing track unavailable — record without it',
          noBackingTrack: 'No backing track available',
          scoreBreakdown: 'Score Breakdown',
          pitchAccuracy: 'Pitch Accuracy',
          rhythmTiming: 'Rhythm Timing',
          voiceQuality: 'Voice Quality',
          weightExplanation: 'Pitch {{pitch}}% • Rhythm {{rhythm}}% • Voice {{voice}}%',
          notAvailable: 'N/A',
          performanceSummary: 'Performance Summary',
        },
        tapToRecord: 'Tap to Record',
        recording: 'Recording...',
        reviewRecording: 'Review your recording',
        listenFirst: 'Listen to the reference audio first',
        readyToRecord: 'Ready to Record',        reRecord: 'Record Again',
        submitRecording: 'Submit Recording',
        timeRemaining: '{{seconds}}s remaining',
        listenAtLeastOnce: 'Listen to the reference at least once before recording',
        timerStartsAfterListen: 'Timer starts when you begin recording',
        customTimeLimit: 'Custom Time Limit',
        customTimeLimitHint: 'Audio challenges typically need 60-180 seconds. Leave off to use quest default.',
        prepareYourself: 'Prepare yourself! The timer will start when you begin recording.',
        startRecordingPhase: 'Start Recording Phase',
        backToListening: 'Back to Listening',
        types: {
            RHYTHM_CREATION: {
                label: 'Create Rhythm',
                description: 'Create your own rhythm pattern. Scored on consistency and creativity.',
            },
            RHYTHM_REPEAT: {
                label: 'Repeat Rhythm',
                description: 'Listen and repeat the rhythm pattern you hear.',
            },
            SOUND_MATCH: {
                label: 'Match Sound',
                description: 'Make sounds as close as possible to the reference.',
            },
            SINGING: {
                label: 'Sing Along',
                description: 'Sing the song segment and receive a karaoke-style score.',
            },
        },
        config: {
            title: 'Audio Challenge Configuration',
            referenceAudio: 'Reference Audio *',
            uploadButton: 'Upload Audio File',
            segmentOptional: 'Audio Segment (optional)',
            startSec: 'Start (sec)',
            endSec: 'End (sec)',
            fullPlaceholder: 'Full',
            passScore: 'Minimum Score to Pass: {{percentage}}%',
            rhythmSettings: 'Rhythm Settings',
            bpm: 'BPM:',
            time: 'Time:',
            errorPick: 'Failed to select audio file',
        },
        header: {
            title: 'Audio Challenge',
            defaultDescription: 'Complete the audio task',
            passScore: 'Pass Score: {{percentage}}%',
        },
        selector: {
            label: 'Challenge Type *',
            pitch: 'Pitch',
            rhythm: 'Rhythm',
            voice: 'Voice',
            needsAudio: 'Needs audio',
        },
    },
    audioPlayer: {
        loading: 'Loading audio...',
        error: 'Failed to load audio',
        retry: 'Tap to retry',
        noAudio: 'Audio not available',
        durationPlaceholder: '--:--',
    },
    wwwGame: {
        // --- EXISTING (keep as-is) ---
        performance: {
            outstanding: 'Outstanding! Your team showcased exceptional knowledge!',
            great: 'Great job! Your team has impressive knowledge!',
            good: 'Good effort! Your team did well!',
            niceTry: "Nice try! Keep learning and you'll improve!",
            dontGiveUp: "Don't give up! Every game is a learning opportunity!",
        },
        feedback: {
            discussedCorrect: 'I heard the correct answer "{{answer}}" during your discussion, but it wasn\'t your final answer.',
            notDiscussedCorrect: 'The correct answer was "{{answer}}". I didn\'t hear this answer during your discussion.',
            correct: 'Great job! "{{answer}}" is correct!',
        },
        hint: {
            characters: 'The answer contains {{count}} characters.',
            words: " It's {{count}} words.",
        },
        // --- NEW SUB-SECTION ---
        gameFeedback: {
            strongestPlayer: '{{player}} was your strongest player, answering {{correct}} out of {{total}} questions correctly. ',
            struggledWith: 'The team struggled most with questions about ',
            studyMore: '. Consider studying these topics more for next time!',
            excellentKnowledge: 'Your team showed excellent knowledge across all question categories!',
        },
    },
    multiplayer: {
        gameOver: {
            title: 'Game Over!',
            yourRank: 'YOUR RANK',
            points: '{{count}} pts',
            leaderboard: 'Leaderboard',
            returnHome: 'RETURN HOME',
        },
    },
    wwwPhases: {
        // --- EXISTING (keep as-is) ---
        mediaPlayback: {
            watchVideo: 'Watch the Video',
            listenAudio: 'Listen to the Audio',
            replay: 'Replay',
            continueToDiscussion: 'Continue to Discussion',
            skipMedia: 'Skip Media',
            mediaUnavailable: 'Media is not available for this question.',
        },
        waiting: {
            readyToStart: 'Ready to Start?',
            startQuiz: 'Start Quiz',
            starting: 'Starting...', 
            preparingMedia: 'Preparing media...',
            mediaReady: 'Ready to play!',
            // --- NEW KEY ---
            welcomeText: "Welcome to WWW Quiz!\n\nYou'll have {{roundTime}} seconds to discuss each question with your team.\n\nWhen you're ready to begin, press the button below.",
        },
        youtube: {
            unavailable: 'YouTube video unavailable',
            retry: 'Retry',
            replay: 'Replay',
            skipBack: 'Back 10s',
            skipForward: 'Forward 10s',
            rewind10: 'Rewind 10 seconds',
            forward10: 'Forward 10 seconds',
            playPause: 'Play or pause',
        },
        discussion: {
            discussionNotes: 'Discussion Notes:',
            notesPlaceholder: "Record your team's discussion...",
            proceedToAnswer: 'Proceed to Answer',
            latestTranscription: 'Latest Transcription:',
            timeRemaining: 'Time Remaining: {{seconds}}s',
        },

        // --- NEW SUB-SECTION ---
        feedback: {
            title: 'Round Results',
            question: 'Question:',
            yourAnswer: 'Your Answer:',
            correctAnswer: 'Correct Answer:',
            correct: 'CORRECT',
            incorrect: 'INCORRECT',
            aiHostFeedback: 'AI Host Feedback:',
            finishGame: 'Finish Game',
            nextQuestion: 'Next Question',
        },
        audioAnswer: {
            title: 'Answer the Question',
            listenToQuestion: 'Listen to the audio question',
            replay: 'Replay Audio',
            replaying: 'Playing...', 
            timeRemaining: '{{seconds}}s remaining',
            answerPlaceholder: 'Type your answer...', 
            submitAnswer: 'Submit Answer',
            submitting: 'Submitting...', 
            recordYourAnswer: 'Record Your Answer',
            tapToRecord: 'Tap to start recording',
            stopRecording: 'Stop Recording',
            recordAgain: 'Record Again',
            audioNotAvailable: 'Audio not available',
            listenAgain: 'Listen Again',
            replaysRemaining: '{{count}} replays left',
            unlimitedReplays: 'Unlimited Replays',
            noReplaysLeft: 'No Replays Left',
            listenToQuestionFirst: 'Listen to the question first',
            reviewYourAnswer: 'Review Your Recording',
            scoring: {
                analyzing: 'Scoring your performance...',
                overallScore: 'Score',
                pitchScore: 'Pitch',
                rhythmScore: 'Rhythm',
                voiceScore: 'Voice',
                passed: 'PASSED',
                failed: 'TRY AGAIN',
                reactionTime: 'Reaction: {{ms}}ms',
                reactionFast: 'Lightning fast!',
                reactionGood: 'Good timing!',
                reactionSlow: 'A bit slow',
                scoringFailed: 'Scoring failed. You can still submit your answer.',
                retry: 'Retry Scoring',
                submitWithoutScore: 'Submit Without Score',
                listenFirst: 'Listen to the reference audio first',
                readyToRecord: 'Ready to Record',
                extractingPattern: 'Analyzing reference audio...',
            },
            startTimer: 'Start',
            answerModeText: 'Type Answer',
            answerModeRecord: 'Record Answer',
        },
        answer: {
            teamAnswer: 'Team Answer:',
            whoIsAnswering: 'Who is answering?',
            submitAnswer: 'Submit Answer',
            submitting: 'Submitting...', 
            enterAnswer: "Enter your team's answer...",
            stopRecordingAnswer: 'Stop Recording Answer',
            recordVoiceAnswer: 'Record Voice Answer',
            startTyping: 'Start typing to begin countdown...', 
            timeToComplete: 'Time to complete: {{seconds}}s',
        },
    },
    tvDisplay: {
        connect: {
            title: 'Connect TV Display',
            instruction: 'Enter the code shown on your TV',
            placeholder: 'TV CODE',
            button: 'Connect',
            success: 'TV connected!',
            error: {
                invalid: 'Invalid code. Check your TV and try again.',
                expired: 'Code expired. Check your TV for a new code.',
                alreadyClaimed: 'This TV is already connected to a room.',
                unknown: 'Failed to connect TV display.',
            },
        },
    },
  soloQuest: {
    feed: {
      title: 'Solo Quests',
      empty: 'No quests found nearby. Try adjusting your filters.',
      loadError: 'Failed to load quest feed.',
      filterButton: 'Filter',
      distanceLabel: '{{km}} km away',
      applicantsLabel: '{{count}} applicants',
    },
    status: {
      OPEN: 'Open',
      MATCHED: 'Matched',
      COMPLETED: 'Completed',
      CANCELLED: 'Cancelled',
      EXPIRED: 'Expired',
    },
    depositPolicy: {
      APPLICANT_ONLY: 'Applicant only',
      BOTH_PARTIES: 'Both parties',
      NONE: 'No deposit',
    },
    applicationStatus: {
      PENDING: 'Pending',
      ACCEPTED: 'Accepted',
      DECLINED: 'Declined',
      WITHDRAWN: 'Withdrawn',
      EXPIRED: 'Expired',
    },
    form: {
      titleLabel: 'Title',
      titlePlaceholder: 'e.g. Coffee walk in the park',
      descriptionLabel: 'Description',
      descriptionPlaceholder: 'Tell applicants what this meetup is about',
      locationLabel: 'Meetup location',
      locationPlaceholder: 'Enter location name',
      datetimeLabel: 'Meetup date & time',
      targetGenderLabel: 'Preferred gender',
      ageRangeLabel: 'Age range',
      agePlaceholderMin: 'Min',
      agePlaceholderMax: 'Max',
      interestsLabel: 'Required interests',
      interestsPlaceholder: 'e.g. hiking, coffee',
      maxDistanceLabel: 'Max distance (km)',
      depositPolicyLabel: 'Deposit policy',
      stakeTypeLabel: 'Stake type',
      stakeAmountLabel: 'Stake amount',
      stakeCurrencyLabel: 'Currency',
      socialPenaltyLabel: 'Social penalty description',
      socialPenaltyPlaceholder: 'What happens if someone cancels?',
      submitButton: 'Create Quest',
    },
    application: {
      pitchLabel: 'Your pitch message',
      pitchPlaceholder: 'Tell the creator why you are a great match (10–500 characters)',
      pitchMinLength: 'Pitch must be at least 10 characters',
      pitchMaxLength: 'Pitch must be at most 500 characters',
      applyButton: 'Apply',
      withdrawButton: 'Withdraw',
      acceptButton: 'Accept',
      declineButton: 'Decline',
      counterMessageButton: 'Send message',
      replyButton: 'Reply',
      messageLabel: 'Message (optional)',
      messagePlaceholder: 'Add a note...',
      reasonLabel: 'Decline reason (optional)',
      reasonPlaceholder: 'Let the applicant know why...',
      messageMaxLength: 'Message must be at most 500 characters',
      applySuccess: 'Application sent!',
      applyError: 'Failed to send application.',
      withdrawSuccess: 'Application withdrawn.',
      withdrawError: 'Failed to withdraw application.',
      acceptSuccess: 'Application accepted!',
      acceptError: 'Failed to accept application.',
      declineSuccess: 'Application declined.',
      declineError: 'Failed to decline application.',
      noApplications: 'No applications yet.',
      myApplicationsTitle: 'My Applications',
    },
    checkin: {
      button: 'Check In',
      success: 'Check-in successful!',
      error: 'Check-in failed. Make sure you are at the meetup location.',
      tooFar: 'You are {{distance}}m away from the meetup point.',
      disputeButton: 'Dispute',
      disputeSuccess: 'Dispute submitted.',
      disputeError: 'Failed to submit dispute.',
    },
    reputation: {
      title: 'Reputation',
      score: 'Score: {{score}}',
      marksTitle: 'Reputation Marks',
      noMarks: 'No reputation marks yet.',
      positive: 'Positive',
      negative: 'Negative',
      appealButton: 'Appeal',
      appealReasonLabel: 'Reason for appeal',
      appealReasonPlaceholder: 'Explain why this mark should be removed',
      appealSuccess: 'Appeal submitted.',
      appealError: 'Failed to submit appeal.',
      expiresAt: 'Expires {{date}}',
    },
    errors: {
      loadFailed: 'Failed to load quest.',
      createFailed: 'Failed to create quest.',
      cancelFailed: 'Failed to cancel quest.',
      notFound: 'Quest not found.',
      forbidden: 'You do not have permission to perform this action.',
    },
  },
  soloQuestFeed: {
    title: 'Solo Quests',
    locationLabel: 'Near: {{city}}',
    locationUnknown: 'Location unknown',
    refreshLocation: 'Refresh',
    locationBanner: 'Enable location for better results',
    filterTitle: 'Filter Quests',
    maxDistanceLabel: 'Max distance',
    interestsFilterLabel: 'Filter by interests',
    applyFilter: 'Apply',
    resetFilter: 'Reset',
    distanceUnit: 'km',
    emptyTitle: 'No quests nearby',
    emptyDescription: 'Try increasing your search radius or create your own!',
    createQuestButton: 'Create a Quest',
    errorTitle: 'Failed to load quests',
    retry: 'Retry',
    loadingMore: 'Loading more...',
    free: 'Free',
    depositPoints: '{{amount}} pts deposit',
    depositMoney: '{{amount}} deposit',
    applicantsCount: '{{count}} applicants',
    reputation: '★ {{score}}',
  },
  editProfileDetails: {
    title: 'Edit Extended Profile',
    save: 'Save',
    saving: 'Saving...',
    dateOfBirthLabel: 'Date of Birth',
    dateOfBirthPlaceholder: 'Select your date of birth',
    relationshipStatusLabel: 'Relationship Status',
    relationshipStatusPlaceholder: 'Select status',
    cityLabel: 'City',
    cityPlaceholder: 'Enter your city',
    useMyLocation: 'Use my location',
    locating: 'Locating...',
    aboutMeLabel: 'About Me',
    aboutMePlaceholder: 'Tell others about yourself...',
    interestsLabel: 'Interests',
    interestsPlaceholder: 'Type an interest and press Enter',
    saveSuccess: 'Profile details updated',
    saveError: 'Failed to save profile details',
    locationError: 'Could not determine your location',
    validation: {
      dobRequired: 'Date of birth is required',
      dobTooYoung: 'You must be at least 18 years old',
      dobFuture: 'Date of birth cannot be in the future',
      aboutMeTooLong: 'About me must be at most 500 characters',
      tagTooLong: 'Each interest must be at most 30 characters',
      tooManyTags: 'You can add at most 20 interests',
      cityTooLong: 'City name must be at most 100 characters',
    },
    relationshipStatus: {
      SINGLE: 'Single',
      IN_RELATIONSHIP: 'In a relationship',
      MARRIED: 'Married',
      DIVORCED: 'Divorced',
      WIDOWED: 'Widowed',
      PREFER_NOT_TO_SAY: 'Prefer not to say',
    },
  },
  extendedProfile: {
    sectionTitle: 'Extended Profile',
    age: '{{age}} years old',
    city: 'City: {{city}}',
    aboutMe: 'About me',
    interests: 'Interests',
    editButton: 'Edit Extended Profile',
    ctaTitle: 'Complete your profile',
    ctaDescription: 'Complete your profile to discover Solo Quests',
    ctaButton: 'Fill in profile',
  },
  createSoloQuest: {
    title: 'Create Solo Quest',
    sections: {
      what: 'What',
      when: 'When',
      where: 'Where',
      who: 'Who (optional)',
      deposit: 'Deposit & Penalty',
    },
    useMyLocation: 'Use my location',
    locating: 'Locating...',
    showManualCoords: 'Enter coordinates manually',
    latLabel: 'Latitude',
    lngLabel: 'Longitude',
    latPlaceholder: 'e.g. 52.3676',
    lngPlaceholder: 'e.g. 4.9041',
    targetGender: {
      ANY: 'Any',
      MALE: 'Male',
      FEMALE: 'Female',
    },
    relationshipStatusLabel: 'Relationship status',
    relationshipStatusAny: 'Any',
    stakeType: {
      POINTS: 'Points',
      MONEY: 'Money',
      SOCIAL: 'Social Quest',
    },
    creating: 'Creating...',
    successTitle: 'Quest Created!',
    successMessage: 'Your solo quest is now live.',
    viewFeed: 'View Feed',
    validation: {
      titleTooShort: 'Title must be at least 5 characters',
      titleTooLong: 'Title must be at most 100 characters',
      descriptionTooShort: 'Description must be at least 10 characters',
      descriptionTooLong: 'Description must be at most 1000 characters',
      meetupPast: 'Meetup must be at least 1 hour from now',
      locationNameRequired: 'Location name is required',
      locationNameTooLong: 'Location name must be at most 200 characters',
      locationRequired: 'Valid latitude and longitude are required',
      stakeRequired: 'Stake amount must be greater than 0',
      socialPenaltyTooLong: 'Social penalty description must be at most 500 characters',
    },
  },
  soloQuestDetail: {
    title: 'Quest Details',
    applyButton: 'Apply to Quest',
    alreadyApplied: 'Already applied',
    cancelButton: 'Cancel Quest',
    viewApplicationsButton: 'View Applications ({{count}})',
    viewApplicationsButtonEmpty: 'View Applications',
    matchedBanner: "You've been matched!",
    checkInButton: 'Check In',
    pitchModalTitle: 'Apply to Quest',
    pitchLabel: 'Your pitch message',
    pitchPlaceholder: 'Tell the creator why you are a great match (10–500 characters)',
    pitchSubmit: 'Submit Application',
    pitchValidationMin: 'Pitch must be at least 10 characters',
    pitchValidationMax: 'Pitch must be at most 500 characters',
    cancelConfirmTitle: 'Cancel Quest?',
    cancelConfirmMessage: 'Are you sure you want to cancel this quest? This cannot be undone.',
    cancelConfirm: 'Yes, Cancel',
    sections: {
      details: 'Details',
      location: 'Location',
      audience: 'Audience',
      deposit: 'Deposit & Penalty',
      matchedUser: 'Matched With',
    },
    meetupDate: 'Date & Time',
    locationLabel: 'Location',
    targetGenderLabel: 'Preferred gender',
    ageRangeLabel: 'Age range',
    ageRangeValue: '{{min}}–{{max}} years',
    maxDistanceLabel: 'Max distance',
    maxDistanceValue: '{{km}} km',
    relationshipStatusLabel: 'Relationship status',
    requiredInterestsLabel: 'Required interests',
    depositPolicyLabel: 'Deposit policy',
    stakeLabel: 'Stake',
    stakeValue: '{{amount}} {{type}}',
    socialPenaltyLabel: 'Social penalty',
    applicationStatusLabel: 'Your application',
    applySuccess: 'Application submitted!',
    applyError: 'Failed to submit application.',
    cancelSuccess: 'Quest cancelled.',
    cancelError: 'Failed to cancel quest.',
    wagerTitle: 'Wager Info',
  },
  questApplications: {
    title: 'Applications',
    empty: 'No applications yet.',
    ageLabel: '{{age}} years old',
    pitchLabel: 'Pitch',
    aboutMeLabel: 'About',
    creatorMessageLabel: 'Your message',
    applicantReplyLabel: 'Their reply',
    acceptButton: 'Accept',
    declineButton: 'Decline',
    messageButton: 'Message',
    acceptConfirmTitle: 'Accept Application?',
    acceptConfirmMessage: 'This will match the applicant with your quest.',
    declineConfirmTitle: 'Decline Application?',
    declineConfirmMessage: 'The applicant will be notified.',
    messageModalTitle: 'Send Message',
    messagePlaceholder: 'Your message (optional)...',
    declineReasonPlaceholder: 'Reason for declining (optional)...',
    messageSend: 'Send',
    declineSubmit: 'Decline',
    acceptSuccess: 'Application accepted!',
    acceptError: 'Failed to accept application.',
    declineSuccess: 'Application declined.',
    declineError: 'Failed to decline application.',
    messageSent: 'Message sent.',
    messageError: 'Failed to send message.',
    messageMaxLength: 'Message must be at most 500 characters',
  },
  myApplications: {
    title: 'My Applications',
    empty: 'You have no applications yet.',
    withdrawButton: 'Withdraw',
    withdrawConfirmTitle: 'Withdraw Application?',
    withdrawConfirmMessage: 'Are you sure you want to withdraw this application?',
    withdrawSuccess: 'Application withdrawn.',
    withdrawError: 'Failed to withdraw application.',
    pitchLabel: 'Your pitch',
    creatorMessageLabel: 'Creator message',
    yourReplyLabel: 'Your reply',
    questLabel: 'Quest',
    meetupLabel: 'Meetup',
    statusLabel: 'Status',
    tapToView: 'Tap to view quest',
  },
  soloQuestCheckIn: {
    title: 'Check In',
    wagerTitle: 'Wager Info',
    checkInSection: 'Check-In',
    checkInButton: 'Check In Now',
    startsIn: 'Starts in {{time}}',
    locating: 'Getting your location…',
    distanceFromMeetup: '{{distance}} from meetup point',
    permissionDenied: 'Location permission denied. Enable it in Settings.',
    gpsError: 'Could not get your location. Try again in an open area.',
    networkError: 'Network error. Please try again.',
    tooFar: 'You are too far from the meetup point.',
    waitingOther: 'Checked in! Waiting for the other party…',
    bothConfirmed: 'Both confirmed! Have fun! 🎉',
    matchedUser: 'Matched User',
    depositAmount: 'Deposit Amount',
    reportIssue: 'Report Issue',
    disputeModalTitle: 'Report an Issue',
    disputeReasonLabel: 'Describe the issue',
    disputeReasonPlaceholder: 'What went wrong? (optional)',
    disputeSubmit: 'Submit Report',
    disputeSuccess: 'Report submitted.',
    disputeError: 'Failed to submit report.',
  },
  screenLock: {
    title: 'Screen Time Expired',
    budgetExhaustedReason: 'Your screen time has been used up. The device will unlock when the next period begins.',
    penaltyReason: 'A penalty is active: {{reason}}',
    settingsButton: 'Screen Time Settings',
    notificationTitle: 'Screen Time Limit Active',
    notificationMessage: '{{minutes}} minutes remaining',
    permissionTitle: 'Permission Required',
    permissionMessage: 'Screen Time is enabled on your account. To enforce screen time limits when your budget runs out, RhythmGame needs the "Draw over other apps" permission. This will only be used when Screen Time tracking is active.',
    permissionButton: 'Grant Permission',
  },
};

export default en;
